...
typedef struct {
    FILE_TYPE file_type;        // Ο τύπος του αρχείου
    int file_descriptor;        // Ο αναγνωριστικός αριθμός ανοίγματος αρχείου απο το επίπεδο block
    int block_id;               // Το id του block στο οποίο είναι αποθηκευμένο
    long int num_of_buckets;    // το πλήθος των “κάδων” του αρχείου κατακερματισμού
    int *hash_table;            // Ο πίνακας κατακερματισμού
    int max_pairs;              // Ο αριθμός των ζυγαριών που χωράνε σε κάθε BF_Block 
} SHT_info;

typedef struct {
    int block_id;               // Το id του block
    int num_of_pairs;           // Ο αριθμός των ζευγαριών στο συγκεκριμένο block
    int overflow_block_id;      // Το id του block υπερχείλισης
} SHT_block_info;

typedef struct {
    char name[15];              // Το όνομα της εγγραφής
    int block_id;               // Το id του block στο οποίο είναι αποθηκευμένη η εγγραφή
} SHT_pair;
